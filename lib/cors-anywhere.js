...
function getHandler(options, proxy) {
  var corsAnywhere = {
    ...
    isValidUrlToBeProxied: null,    // Function that checks the url that needs to be proxied.
    isValidAcceptHeader: null,      // Function that checks the accept header.
  ...
  return function(req, res) {
    ...
    if (corsAnywhere.isValidUrlToBeProxied && !corsAnywhere.isValidUrlToBeProxied(req.url.substring(1))) {
      res.writeHead(400, 'Invalid url to be proxied', req.url.substring(1));
      res.end();
      return;
    }

    if (corsAnywhere.isValidAcceptHeader && !corsAnywhere.isValidAcceptHeader(req)) {
      res.writeHead(400, 'The Accept header contains invalid content types');
      res.end();
      return;
    }
